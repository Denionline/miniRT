...
/*   Updated: 2026/02/01 19:16:53 by dximenes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// q[0]=a, q[1]=b, q[2]=c, q[3]=delta -> Bhaskara
static t_intersections	intersect_cone(t_object *cone, t_ray r,
		t_intersection *xs)
{
	float	q[4];
	float	t[2];
	...
	count = 0;
	q[0] = powf(r.direction.x, 2) - powf(r.direction.y, 2) + powf(
			r.direction.z, 2);
	q[1] = 2.0f * (r.origin.x
			* r.direction.x
			- r.origin.y * r.direction.y + r.origin.z * r.direction.z);
